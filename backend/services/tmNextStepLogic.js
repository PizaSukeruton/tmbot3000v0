...
class NextStepLogicFilter {
  ...
  assessContextNeed(intent, query) {
    console.log("[NEXTSTEP] Assessing context need for:", intent.intent_type, "in set?", this.contextDependentIntents.has(intent.intent_type));
    
    // venue_query with location does NOT need context
    if (intent.intent_type === 'venue_query' && intent.entities && intent.entities.location) {
      return false;
    }
    
    // Check if it's a known context-dependent intent
  ...
  hasAdequateContext(query, entities, sessionContext, intent) {
    console.log("[NEXTSTEP] Checking adequate context - query:", query, "entities:", entities, "sessionContext:", sessionContext);
    
    // For venue_query, if location is provided, that's sufficient context
    if (entities && entities.query_type && entities.location) {
      console.log("[NEXTSTEP] Venue query has location, context is adequate");
      return true;
    }
    
    // For personnel queries, check if location is already specified
    if (intent && intent.intent_type === "personnel_query") {
