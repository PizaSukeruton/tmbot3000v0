    ...
    try {
      ...
      console.log("[DEBUG] Intent received:", JSON.stringify(intent));      console.log("[DEBUG] Intent received:", JSON.stringify(intent));      switch (intent.intent_type) {
        ...
        case "venue_query": {
          ...
          if (queryLocation) {
            // Find show for the mentioned location
          ...
          const venue = await dataSource.getVenue(context.venue_id);
            console.log("[AI] Venue object:", JSON.stringify(venue, null, 2));          
          if (!venue) {
          ...
          switch (queryType) {
            case "contact":
              response = `The venue contact for ${venue.name} is ${venue.contact.name} - ${venue.contact.phone} (${venue.contact.email}).`;
              break;
            case "phone":
              response = `The venue phone for ${venue.name} is ${venue.phone}.`;
              break;
            case "email":
              response = `The venue contact email for ${venue.name} is ${venue.contact.email}.`;
              break;
            default:
